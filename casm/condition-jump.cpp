...
ConditionJump::ConditionJump(const Ceng::UINT32 position,
	const Casm::CONDITION::value condition, const CodeLabel* label)
...
Ceng::CRESULT ConditionJump::AdjustOffset(X86::BuildParams *params,const Ceng::INT32 offset)
{
	if (jumpSize == X86::OPERAND_SIZE::BYTE)
	{
		...
		else
		{
			...
			X86::ImmediateOperand temp(jumpSize, newOffset);

			cresult = jumpCommand->EncodeOneForm(params,codeBuffer,&temp);
								   ...
								   const std::vector<std::shared_ptr<CodeElement>> &codeList)
{
	...
	if (target->Position() > position)
	{
		...
		for(k = position+1; k < target->Position();k++)
		{
			...
			{
				ConditionJump *jump = (ConditionJump*)(codeList[k].get());
	...
	X86::ImmediateOperand temp(jumpSize, offset);
